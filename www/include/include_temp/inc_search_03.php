
								<div class="sz-list">
									<ul class="grid">
										<!-- 리스트 반복 -->
										<li class="grid-item col-md-4">
											<div class="sz-list-border">

												<div class="sz-list-header">
													<!-- 상장 내용 -->
													<div class="sz-img">
														<a href="./sz_view.php">
															<img src="./assets/img/temp_sz.png" alt="받은 상장"/>
														</a>
													</div>
													<!-- 상장 내용 End -->

													<!-- 사진 -->
													<div id="photo" class="photo-content">
														<!-- 포토 리스트 -->
														<div class="photo-list">
															<ul class="row">
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/o.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/h.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/g.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/x.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/z.gif" alt="">
																	</div>
																</li>
															</ul>
														</div>
														<!-- 포토 리스트 End -->
													</div>
													<!-- 사진 End -->
												</div>

												<div class="sz-list-body">
													<!-- 작성자 -->
													<div class="user">
														<div class="user-img">
															<a href="#;"><img src="assets/tempImg/profile/hbs.jpg" alt="프로필 이미지">
															</a>
														</div>
														<div class="user-nickname">
															<a href="#;">bongsik</a>
														</div>
														<div class="user-more">
															<div class="tooltip small right bottom">
																<a href="#;" class="trigger"><img class="ico-more" src="./assets/img/ico-more.png" alt=""></a>
																<div class="layer">
																	<ul>
																		<li><a href="#;">수정</a></li>
																		<li><a href="#;">신고</a></li>
																	</ul>
																</div>
															</div>
														</div>
													</div>
													<!-- 작성자 End -->

													<!-- 내용 -->
													<div class="info">
														<div class="story">
															볼 때마다 아직 인류가 받아들이기엔 이른 패션센스 같아 상을 줘야겠구나 치명상!응 신중하게 서두르자!청각장애인도 들리겠다.작년에 선배 결혼하는데, 뷔페에 연어 있길래 나는 알래스카 곰새끼다 생각하면서 연허 한 6점신가 가져다 먹었는데
														</div>
														<div class="hashtag">
															<h3 class="blind">해시태그</h3>
															<div class="hashtag-content">
																<a href="#;">#상장</a>, <a href="#;">#감사</a>, <a href="#;">#친구</a>, <a href="#;">#게임</a>
															</div>
														</div>

														<div class="user send-with">
															<a href="#;" class="with-send">
																<ul>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/ag.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/s.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/l.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/f.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/z.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																</ul>
																<span class="with-num">20</span>
															</a>
														</div>

														<div class="sz-function-btn">
															<div class="sz-function sz-function-02">
																<a href="#;">
																	<span class="love">
																		<img class="ico-love" src="./assets/img/ico-loveit-on.png" alt="love it!"> <span class="nums">7,198</span>
																	</span>
																</a>
																<a href="#communication">
																	<span class="commucation">
																		<img class="ico-comment" src="./assets/img/ico-comment.png" alt="reply"> <span class="nums">624</span>
																	</span>
																</a>
																<a href="#photo">
																	<span class="photo">
																		<img class="ico-photo" src="./assets/img/ico-photo.png" alt="photo"> <span class="nums">5</span>
																	</span>
																</a>
																<!--
																<a href="#;">
																	<span  class="movie">
																		<img src="./assets/img/ico-movie.png" alt="movie"/>
																	</span>
																</a>
																-->
															</div>
														</div>
													</div>
													<!-- 내용 End -->
												</div>
											</div>
										</li>
										<!-- 리스트 반복 End -->

										<!-- 리스트 반복 -->
										<li class="grid-item col-md-4">
											<div class="sz-list-border">

												<div class="sz-list-header">
													<!-- 상장 내용 -->
													<div class="sz-img">
														<a href="./sz_view.php">
															<img src="./assets/img/temp_sz.png" alt="받은 상장"/>
														</a>
													</div>
													<!-- 상장 내용 End -->

													<!-- 사진 -->
													<div id="photo" class="photo-content">
														<!-- 포토 리스트 -->
														<div class="photo-list">
															<ul class="row">
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/o.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/h.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/g.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/x.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/z.gif" alt="">
																	</div>
																</li>
															</ul>
														</div>
														<!-- 포토 리스트 End -->
													</div>
													<!-- 사진 End -->
												</div>

												<div class="sz-list-body">
													<!-- 작성자 -->
													<div class="user">
														<div class="user-img">
															<a href="#;"><img src="assets/tempImg/profile/hbs.jpg" alt="프로필 이미지">
															</a>
														</div>
														<div class="user-nickname">
															<a href="#;">bongsik</a>
														</div>
														<div class="user-more">
															<div class="tooltip small right bottom">
																<a href="#;" class="trigger"><img class="ico-more" src="./assets/img/ico-more.png" alt=""></a>
																<div class="layer">
																	<ul>
																		<li><a href="#;">수정</a></li>
																		<li><a href="#;">신고</a></li>
																	</ul>
																</div>
															</div>
														</div>
													</div>
													<!-- 작성자 End -->

													<!-- 내용 -->
													<div class="info">
														<div class="story">
															볼 때마다 아직 인류가 받아들이기엔 이른 패션센스 같아 상을 줘야겠구나 치명상!응 신중하게 서두르자!청각장애인도 들리겠다.작년에 선배 결혼하는데, 뷔페에 연어 있길래 나는 알래스카 곰새끼다 생각하면서 연허 한 6점신가 가져다 먹었는데
														</div>
														<div class="hashtag">
															<h3 class="blind">해시태그</h3>
															<div class="hashtag-content">
																<a href="#;">#상장</a>, <a href="#;">#감사</a>, <a href="#;">#친구</a>, <a href="#;">#게임</a>
															</div>
														</div>

														<div class="user send-with">
															<a href="#;" class="with-send">
																<ul>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/ag.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/s.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/l.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/f.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/z.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																</ul>
																<span class="with-num">20</span>
															</a>
														</div>

														<div class="sz-function-btn">
															<div class="sz-function sz-function-02">
																<a href="#;">
																	<span class="love">
																		<img class="ico-love" src="./assets/img/ico-loveit-on.png" alt="love it!"> <span class="nums">7,198</span>
																	</span>
																</a>
																<a href="#communication">
																	<span class="commucation">
																		<img class="ico-comment" src="./assets/img/ico-comment.png" alt="reply"> <span class="nums">624</span>
																	</span>
																</a>
																<a href="#photo">
																	<span class="photo">
																		<img class="ico-photo" src="./assets/img/ico-photo.png" alt="photo"> <span class="nums">5</span>
																	</span>
																</a>
																<!--
																<a href="#;">
																	<span  class="movie">
																		<img src="./assets/img/ico-movie.png" alt="movie"/>
																	</span>
																</a>
																-->
															</div>
														</div>
													</div>
													<!-- 내용 End -->
												</div>
											</div>
										</li>
										<!-- 리스트 반복 End -->
										<!-- 리스트 반복 -->
										<li class="grid-item col-md-4">
											<div class="sz-list-border">

												<div class="sz-list-header">
													<!-- 상장 내용 -->
													<div class="sz-img">
														<a href="./sz_view.php">
															<img src="./assets/img/temp_sz.png" alt="받은 상장"/>
														</a>
													</div>
													<!-- 상장 내용 End -->

													<!-- 사진 -->
													<div id="photo" class="photo-content">
														<!-- 포토 리스트 -->
														<div class="photo-list">
															<ul class="row">
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/o.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/h.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/g.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/x.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/z.gif" alt="">
																	</div>
																</li>
															</ul>
														</div>
														<!-- 포토 리스트 End -->
													</div>
													<!-- 사진 End -->
												</div>

												<div class="sz-list-body">
													<!-- 작성자 -->
													<div class="user">
														<div class="user-img">
															<a href="#;"><img src="assets/tempImg/profile/hbs.jpg" alt="프로필 이미지">
															</a>
														</div>
														<div class="user-nickname">
															<a href="#;">bongsik</a>
														</div>
														<div class="user-more">
															<div class="tooltip small right bottom">
																<a href="#;" class="trigger"><img class="ico-more" src="./assets/img/ico-more.png" alt=""></a>
																<div class="layer">
																	<ul>
																		<li><a href="#;">수정</a></li>
																		<li><a href="#;">신고</a></li>
																	</ul>
																</div>
															</div>
														</div>
													</div>
													<!-- 작성자 End -->

													<!-- 내용 -->
													<div class="info">
														<div class="story">
															볼 때마다 아직 인류가 받아들이기엔 이른 패션센스 같아 상을 줘야겠구나 치명상!응 신중하게 서두르자!청각장애인도 들리겠다.작년에 선배 결혼하는데, 뷔페에 연어 있길래 나는 알래스카 곰새끼다 생각하면서 연허 한 6점신가 가져다 먹었는데
														</div>
														<div class="hashtag">
															<h3 class="blind">해시태그</h3>
															<div class="hashtag-content">
																<a href="#;">#상장</a>, <a href="#;">#감사</a>, <a href="#;">#친구</a>, <a href="#;">#게임</a>
															</div>
														</div>

														<div class="user send-with">
															<a href="#;" class="with-send">
																<ul>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/ag.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/s.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/l.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/f.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/z.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																</ul>
																<span class="with-num">20</span>
															</a>
														</div>

														<div class="sz-function-btn">
															<div class="sz-function sz-function-02">
																<a href="#;">
																	<span class="love">
																		<img class="ico-love" src="./assets/img/ico-loveit-on.png" alt="love it!"> <span class="nums">7,198</span>
																	</span>
																</a>
																<a href="#communication">
																	<span class="commucation">
																		<img class="ico-comment" src="./assets/img/ico-comment.png" alt="reply"> <span class="nums">624</span>
																	</span>
																</a>
																<a href="#photo">
																	<span class="photo">
																		<img class="ico-photo" src="./assets/img/ico-photo.png" alt="photo"> <span class="nums">5</span>
																	</span>
																</a>
																<!--
																<a href="#;">
																	<span  class="movie">
																		<img src="./assets/img/ico-movie.png" alt="movie"/>
																	</span>
																</a>
																-->
															</div>
														</div>
													</div>
													<!-- 내용 End -->
												</div>
											</div>
										</li>
										<!-- 리스트 반복 End -->
										<!-- 리스트 반복 -->
										<li class="grid-item col-md-4">
											<div class="sz-list-border">

												<div class="sz-list-header">
													<!-- 상장 내용 -->
													<div class="sz-img">
														<a href="./sz_view.php">
															<img src="./assets/img/temp_sz.png" alt="받은 상장"/>
														</a>
													</div>
													<!-- 상장 내용 End -->

													<!-- 사진 -->
													<div id="photo" class="photo-content">
														<!-- 포토 리스트 -->
														<div class="photo-list">
															<ul class="row">
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/o.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/h.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/g.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/x.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/z.gif" alt="">
																	</div>
																</li>
															</ul>
														</div>
														<!-- 포토 리스트 End -->
													</div>
													<!-- 사진 End -->
												</div>

												<div class="sz-list-body">
													<!-- 작성자 -->
													<div class="user">
														<div class="user-img">
															<a href="#;"><img src="assets/tempImg/profile/hbs.jpg" alt="프로필 이미지">
															</a>
														</div>
														<div class="user-nickname">
															<a href="#;">bongsik</a>
														</div>
														<div class="user-more">
															<div class="tooltip small right bottom">
																<a href="#;" class="trigger"><img class="ico-more" src="./assets/img/ico-more.png" alt=""></a>
																<div class="layer">
																	<ul>
																		<li><a href="#;">수정</a></li>
																		<li><a href="#;">신고</a></li>
																	</ul>
																</div>
															</div>
														</div>
													</div>
													<!-- 작성자 End -->

													<!-- 내용 -->
													<div class="info">
														<div class="story">
															볼 때마다 아직 인류가 받아들이기엔 이른 패션센스 같아 상을 줘야겠구나 치명상!응 신중하게 서두르자!청각장애인도 들리겠다.작년에 선배 결혼하는데, 뷔페에 연어 있길래 나는 알래스카 곰새끼다 생각하면서 연허 한 6점신가 가져다 먹었는데
														</div>
														<div class="hashtag">
															<h3 class="blind">해시태그</h3>
															<div class="hashtag-content">
																<a href="#;">#상장</a>, <a href="#;">#감사</a>, <a href="#;">#친구</a>, <a href="#;">#게임</a>
															</div>
														</div>

														<div class="user send-with">
															<a href="#;" class="with-send">
																<ul>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/ag.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/s.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/l.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/f.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/z.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																</ul>
																<span class="with-num">20</span>
															</a>
														</div>

														<div class="sz-function-btn">
															<div class="sz-function sz-function-02">
																<a href="#;">
																	<span class="love">
																		<img class="ico-love" src="./assets/img/ico-loveit-on.png" alt="love it!"> <span class="nums">7,198</span>
																	</span>
																</a>
																<a href="#communication">
																	<span class="commucation">
																		<img class="ico-comment" src="./assets/img/ico-comment.png" alt="reply"> <span class="nums">624</span>
																	</span>
																</a>
																<a href="#photo">
																	<span class="photo">
																		<img class="ico-photo" src="./assets/img/ico-photo.png" alt="photo"> <span class="nums">5</span>
																	</span>
																</a>
																<!--
																<a href="#;">
																	<span  class="movie">
																		<img src="./assets/img/ico-movie.png" alt="movie"/>
																	</span>
																</a>
																-->
															</div>
														</div>
													</div>
													<!-- 내용 End -->
												</div>
											</div>
										</li>
										<!-- 리스트 반복 End -->
										<!-- 리스트 반복 -->
										<li class="grid-item col-md-4">
											<div class="sz-list-border">

												<div class="sz-list-header">
													<!-- 상장 내용 -->
													<div class="sz-img">
														<a href="./sz_view.php">
															<img src="./assets/img/temp_sz.png" alt="받은 상장"/>
														</a>
													</div>
													<!-- 상장 내용 End -->

													<!-- 사진 -->
													<div id="photo" class="photo-content">
														<!-- 포토 리스트 -->
														<div class="photo-list">
															<ul class="row">
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/o.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/h.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/g.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/x.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/z.gif" alt="">
																	</div>
																</li>
															</ul>
														</div>
														<!-- 포토 리스트 End -->
													</div>
													<!-- 사진 End -->
												</div>

												<div class="sz-list-body">
													<!-- 작성자 -->
													<div class="user">
														<div class="user-img">
															<a href="#;"><img src="assets/tempImg/profile/hbs.jpg" alt="프로필 이미지">
															</a>
														</div>
														<div class="user-nickname">
															<a href="#;">bongsik</a>
														</div>
														<div class="user-more">
															<div class="tooltip small right bottom">
																<a href="#;" class="trigger"><img class="ico-more" src="./assets/img/ico-more.png" alt=""></a>
																<div class="layer">
																	<ul>
																		<li><a href="#;">수정</a></li>
																		<li><a href="#;">신고</a></li>
																	</ul>
																</div>
															</div>
														</div>
													</div>
													<!-- 작성자 End -->

													<!-- 내용 -->
													<div class="info">
														<div class="story">
															볼 때마다 아직 인류가 받아들이기엔 이른 패션센스 같아 상을 줘야겠구나 치명상!응 신중하게 서두르자!청각장애인도 들리겠다.작년에 선배 결혼하는데, 뷔페에 연어 있길래 나는 알래스카 곰새끼다 생각하면서 연허 한 6점신가 가져다 먹었는데
														</div>
														<div class="hashtag">
															<h3 class="blind">해시태그</h3>
															<div class="hashtag-content">
																<a href="#;">#상장</a>, <a href="#;">#감사</a>, <a href="#;">#친구</a>, <a href="#;">#게임</a>
															</div>
														</div>

														<div class="user send-with">
															<a href="#;" class="with-send">
																<ul>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/ag.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/s.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/l.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/f.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/z.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																</ul>
																<span class="with-num">20</span>
															</a>
														</div>

														<div class="sz-function-btn">
															<div class="sz-function sz-function-02">
																<a href="#;">
																	<span class="love">
																		<img class="ico-love" src="./assets/img/ico-loveit-on.png" alt="love it!"> <span class="nums">7,198</span>
																	</span>
																</a>
																<a href="#communication">
																	<span class="commucation">
																		<img class="ico-comment" src="./assets/img/ico-comment.png" alt="reply"> <span class="nums">624</span>
																	</span>
																</a>
																<a href="#photo">
																	<span class="photo">
																		<img class="ico-photo" src="./assets/img/ico-photo.png" alt="photo"> <span class="nums">5</span>
																	</span>
																</a>
																<!--
																<a href="#;">
																	<span  class="movie">
																		<img src="./assets/img/ico-movie.png" alt="movie"/>
																	</span>
																</a>
																-->
															</div>
														</div>
													</div>
													<!-- 내용 End -->
												</div>
											</div>
										</li>
										<!-- 리스트 반복 End -->
										<!-- 리스트 반복 -->
										<li class="grid-item col-md-4">
											<div class="sz-list-border">

												<div class="sz-list-header">
													<!-- 상장 내용 -->
													<div class="sz-img">
														<a href="./sz_view.php">
															<img src="./assets/img/temp_sz.png" alt="받은 상장"/>
														</a>
													</div>
													<!-- 상장 내용 End -->

													<!-- 사진 -->
													<div id="photo" class="photo-content">
														<!-- 포토 리스트 -->
														<div class="photo-list">
															<ul class="row">
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/o.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/h.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/g.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/x.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/z.gif" alt="">
																	</div>
																</li>
															</ul>
														</div>
														<!-- 포토 리스트 End -->
													</div>
													<!-- 사진 End -->
												</div>

												<div class="sz-list-body">
													<!-- 작성자 -->
													<div class="user">
														<div class="user-img">
															<a href="#;"><img src="assets/tempImg/profile/hbs.jpg" alt="프로필 이미지">
															</a>
														</div>
														<div class="user-nickname">
															<a href="#;">bongsik</a>
														</div>
														<div class="user-more">
															<div class="tooltip small right bottom">
																<a href="#;" class="trigger"><img class="ico-more" src="./assets/img/ico-more.png" alt=""></a>
																<div class="layer">
																	<ul>
																		<li><a href="#;">수정</a></li>
																		<li><a href="#;">신고</a></li>
																	</ul>
																</div>
															</div>
														</div>
													</div>
													<!-- 작성자 End -->

													<!-- 내용 -->
													<div class="info">
														<div class="story">
															볼 때마다 아직 인류가 받아들이기엔 이른 패션센스 같아 상을 줘야겠구나 치명상!응 신중하게 서두르자!청각장애인도 들리겠다.작년에 선배 결혼하는데, 뷔페에 연어 있길래 나는 알래스카 곰새끼다 생각하면서 연허 한 6점신가 가져다 먹었는데
														</div>
														<div class="hashtag">
															<h3 class="blind">해시태그</h3>
															<div class="hashtag-content">
																<a href="#;">#상장</a>, <a href="#;">#감사</a>, <a href="#;">#친구</a>, <a href="#;">#게임</a>
															</div>
														</div>

														<div class="user send-with">
															<a href="#;" class="with-send">
																<ul>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/ag.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/s.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/l.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/f.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/z.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																</ul>
																<span class="with-num">20</span>
															</a>
														</div>

														<div class="sz-function-btn">
															<div class="sz-function sz-function-02">
																<a href="#;">
																	<span class="love">
																		<img class="ico-love" src="./assets/img/ico-loveit-on.png" alt="love it!"> <span class="nums">7,198</span>
																	</span>
																</a>
																<a href="#communication">
																	<span class="commucation">
																		<img class="ico-comment" src="./assets/img/ico-comment.png" alt="reply"> <span class="nums">624</span>
																	</span>
																</a>
																<a href="#photo">
																	<span class="photo">
																		<img class="ico-photo" src="./assets/img/ico-photo.png" alt="photo"> <span class="nums">5</span>
																	</span>
																</a>
																<!--
																<a href="#;">
																	<span  class="movie">
																		<img src="./assets/img/ico-movie.png" alt="movie"/>
																	</span>
																</a>
																-->
															</div>
														</div>
													</div>
													<!-- 내용 End -->
												</div>
											</div>
										</li>
										<!-- 리스트 반복 End -->
										<!-- 리스트 반복 -->
										<li class="grid-item col-md-4">
											<div class="sz-list-border">

												<div class="sz-list-header">
													<!-- 상장 내용 -->
													<div class="sz-img">
														<a href="./sz_view.php">
															<img src="./assets/img/temp_sz.png" alt="받은 상장"/>
														</a>
													</div>
													<!-- 상장 내용 End -->

													<!-- 사진 -->
													<div id="photo" class="photo-content">
														<!-- 포토 리스트 -->
														<div class="photo-list">
															<ul class="row">
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/o.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/h.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/g.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/x.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/z.gif" alt="">
																	</div>
																</li>
															</ul>
														</div>
														<!-- 포토 리스트 End -->
													</div>
													<!-- 사진 End -->
												</div>

												<div class="sz-list-body">
													<!-- 작성자 -->
													<div class="user">
														<div class="user-img">
															<a href="#;"><img src="assets/tempImg/profile/hbs.jpg" alt="프로필 이미지">
															</a>
														</div>
														<div class="user-nickname">
															<a href="#;">bongsik</a>
														</div>
														<div class="user-more">
															<div class="tooltip small right bottom">
																<a href="#;" class="trigger"><img class="ico-more" src="./assets/img/ico-more.png" alt=""></a>
																<div class="layer">
																	<ul>
																		<li><a href="#;">수정</a></li>
																		<li><a href="#;">신고</a></li>
																	</ul>
																</div>
															</div>
														</div>
													</div>
													<!-- 작성자 End -->

													<!-- 내용 -->
													<div class="info">
														<div class="story">
															볼 때마다 아직 인류가 받아들이기엔 이른 패션센스 같아 상을 줘야겠구나 치명상!응 신중하게 서두르자!청각장애인도 들리겠다.작년에 선배 결혼하는데, 뷔페에 연어 있길래 나는 알래스카 곰새끼다 생각하면서 연허 한 6점신가 가져다 먹었는데
														</div>
														<div class="hashtag">
															<h3 class="blind">해시태그</h3>
															<div class="hashtag-content">
																<a href="#;">#상장</a>, <a href="#;">#감사</a>, <a href="#;">#친구</a>, <a href="#;">#게임</a>
															</div>
														</div>

														<div class="user send-with">
															<a href="#;" class="with-send">
																<ul>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/ag.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/s.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/l.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/f.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/z.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																</ul>
																<span class="with-num">20</span>
															</a>
														</div>

														<div class="sz-function-btn">
															<div class="sz-function sz-function-02">
																<a href="#;">
																	<span class="love">
																		<img class="ico-love" src="./assets/img/ico-loveit-on.png" alt="love it!"> <span class="nums">7,198</span>
																	</span>
																</a>
																<a href="#communication">
																	<span class="commucation">
																		<img class="ico-comment" src="./assets/img/ico-comment.png" alt="reply"> <span class="nums">624</span>
																	</span>
																</a>
																<a href="#photo">
																	<span class="photo">
																		<img class="ico-photo" src="./assets/img/ico-photo.png" alt="photo"> <span class="nums">5</span>
																	</span>
																</a>
																<!--
																<a href="#;">
																	<span  class="movie">
																		<img src="./assets/img/ico-movie.png" alt="movie"/>
																	</span>
																</a>
																-->
															</div>
														</div>
													</div>
													<!-- 내용 End -->
												</div>
											</div>
										</li>
										<!-- 리스트 반복 End -->
										<!-- 리스트 반복 -->
										<li class="grid-item col-md-4">
											<div class="sz-list-border">

												<div class="sz-list-header">
													<!-- 상장 내용 -->
													<div class="sz-img">
														<a href="./sz_view.php">
															<img src="./assets/img/temp_sz.png" alt="받은 상장"/>
														</a>
													</div>
													<!-- 상장 내용 End -->

													<!-- 사진 -->
													<div id="photo" class="photo-content">
														<!-- 포토 리스트 -->
														<div class="photo-list">
															<ul class="row">
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/o.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/h.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/g.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/x.gif" alt="">
																	</div>
																</li>
																<li class="col-20p">
																	<div>
																		<img src="assets/tempImg/thumb/z.gif" alt="">
																	</div>
																</li>
															</ul>
														</div>
														<!-- 포토 리스트 End -->
													</div>
													<!-- 사진 End -->
												</div>

												<div class="sz-list-body">
													<!-- 작성자 -->
													<div class="user">
														<div class="user-img">
															<a href="#;"><img src="assets/tempImg/profile/hbs.jpg" alt="프로필 이미지">
															</a>
														</div>
														<div class="user-nickname">
															<a href="#;">bongsik</a>
														</div>
														<div class="user-more">
															<div class="tooltip small right bottom">
																<a href="#;" class="trigger"><img class="ico-more" src="./assets/img/ico-more.png" alt=""></a>
																<div class="layer">
																	<ul>
																		<li><a href="#;">수정</a></li>
																		<li><a href="#;">신고</a></li>
																	</ul>
																</div>
															</div>
														</div>
													</div>
													<!-- 작성자 End -->

													<!-- 내용 -->
													<div class="info">
														<div class="story">
															볼 때마다 아직 인류가 받아들이기엔 이른 패션센스 같아 상을 줘야겠구나 치명상!응 신중하게 서두르자!청각장애인도 들리겠다.작년에 선배 결혼하는데, 뷔페에 연어 있길래 나는 알래스카 곰새끼다 생각하면서 연허 한 6점신가 가져다 먹었는데
														</div>
														<div class="hashtag">
															<h3 class="blind">해시태그</h3>
															<div class="hashtag-content">
																<a href="#;">#상장</a>, <a href="#;">#감사</a>, <a href="#;">#친구</a>, <a href="#;">#게임</a>
															</div>
														</div>

														<div class="user send-with">
															<a href="#;" class="with-send">
																<ul>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/ag.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/s.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/l.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/f.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																	<li>
																		<div class="user-img">
																			<img src="assets/tempImg/profile/z.jpg" alt="프로필 이미지">
																		</div>
																	</li>
																</ul>
																<span class="with-num">20</span>
															</a>
														</div>

														<div class="sz-function-btn">
															<div class="sz-function sz-function-02">
																<a href="#;">
																	<span class="love">
																		<img class="ico-love" src="./assets/img/ico-loveit-on.png" alt="love it!"> <span class="nums">7,198</span>
																	</span>
																</a>
																<a href="#communication">
																	<span class="commucation">
																		<img class="ico-comment" src="./assets/img/ico-comment.png" alt="reply"> <span class="nums">624</span>
																	</span>
																</a>
																<a href="#photo">
																	<span class="photo">
																		<img class="ico-photo" src="./assets/img/ico-photo.png" alt="photo"> <span class="nums">5</span>
																	</span>
																</a>
																<!--
																<a href="#;">
																	<span  class="movie">
																		<img src="./assets/img/ico-movie.png" alt="movie"/>
																	</span>
																</a>
																-->
															</div>
														</div>
													</div>
													<!-- 내용 End -->
												</div>
											</div>
										</li>
										<!-- 리스트 반복 End -->
									</ul>
								</div>